...
class CompatibilityScorer {
  ...
  double _scorePoziom(int poziomKlienta, MatchCriteria poziomNartyInfo) {
    if (poziomNartyInfo.status == 'green' &&
        poziomNartyInfo.opis.contains('OK')) {
  ...
  double _scoreWaga(int wagaKlienta, MatchCriteria wagaNartyInfo) {
    if (wagaNartyInfo.status == 'green') {
      ...
      final wagaMin = (int.tryParse(wagaNartyInfo.dodatkoweInfo1 ?? '0') ?? 0)
          .toDouble();
      final wagaMax = (int.tryParse(wagaNartyInfo.dodatkoweInfo2 ?? '0') ?? 0)
          .toDouble();
      final wagaSrodek = (wagaMin + wagaMax) / 2;
      return _gaussianScore(
        wagaKlienta.toDouble(),
        wagaSrodek,
        _tolerancje['waga']!,
      );
  ...
  double _scoreWzrost(int wzrostKlienta, MatchCriteria wzrostNartyInfo) {
    if (wzrostNartyInfo.status == 'green') {
      // Oblicz jak blisko środka zakresu jest klient
      final wzrostMin =
          (int.tryParse(wzrostNartyInfo.dodatkoweInfo1 ?? '0') ?? 0).toDouble();
      final wzrostMax =
          (int.tryParse(wzrostNartyInfo.dodatkoweInfo2 ?? '0') ?? 0).toDouble();
      final wzrostSrodek = (wzrostMin + wzrostMax) / 2;
      return _gaussianScore(
        wzrostKlienta.toDouble(),
        wzrostSrodek,
        _tolerancje['wzrost']!,
      );
  ...
  double _scorePlec(String plecKlienta, MatchCriteria plecNartyInfo) {
    if (plecNartyInfo.status == 'green' && plecNartyInfo.opis.contains('OK')) {
      ...
      if (plecNartyInfo.opis.contains('Narta męska') ||
          plecNartyInfo.opis.contains('Narta kobieca')) {
  ...
  double _scorePrzeznaczenie(
    String? stylKlienta,
    MatchCriteria przeznaczenieNartyInfo,
  ) {
    if (stylKlienta == null || stylKlienta == "Wszystkie") {
      return 1.0; // Brak preferencji = pełny wynik
    }

    if (przeznaczenieNartyInfo.status == 'green' &&
        przeznaczenieNartyInfo.opis.contains('OK')) {
  ...
  Map<String, dynamic> obliczWspolczynnikIdealnosci(
    ...
    if (dopasowanie.containsKey('poziom')) {
      wynikiKryteriow['poziom'] = _scorePoziom(
        poziomKlienta,
        dopasowanie['poziom']!,
      );
    }
    ...
    if (dopasowanie.containsKey('wzrost')) {
      wynikiKryteriow['wzrost'] = _scoreWzrost(
        wzrostKlienta,
        dopasowanie['wzrost']!,
      );
    }
    ...
    if (dopasowanie.containsKey('przeznaczenie')) {
      wynikiKryteriow['przeznaczenie'] = _scorePrzeznaczenie(
        stylKlienta,
        dopasowanie['przeznaczenie']!,
      );
    }
